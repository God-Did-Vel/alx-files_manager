...
export default class FilesController {
  ...
  static async postUpload(req, res) {
    ...
    const newFile = {
OAOAOAOAOAOAOAOAOA      userId: new mongoDBCore.BSON.ObjectId(userId),
      name,
OAOAOA      type,
      isPublic,
...
OAOAOA      newFile.localPath = localPath;
    }
OAOAOA    const insertionInfo = await (await dbClient.filesCollection())
      .insertOne(newFile);
    const fileId = insertionInfo.insertedId.toString();
OAOAOA    // start thumbnail generation worker
OAOAOA    if (type === VALID_FILE_TYPES.image) {
...
OAOAOA    }
  ...
  static async getShow(req, res) {
    ...
    const userId = user._id.toString();
OAOAOA    const file = await (await dbClient.filesCollection())
OAOAOAOAOAOA      .findOne({
        _id: new mongoDBCore.BSON.ObjectId(isValidId(id) ? id : NULL_ID),
OAOAOA        userId: new mongoDBCore.BSON.ObjectId(isValidId(userId) ? userId : NULL_ID),
      });
...
OAOAOA    res.status(200).json({
      id,
      userId,
      name: file.name,
      type: file.type,
      isPublic: file.isPublic,
OAOAOAOAOAOA      parentId: file.parentId === ROOT_FOLDER_ID.toString()
OAOAOA        ? 0
...
OAOAOA  /**
  ...
  static async getIndex(req, res) {
    const { user } = req;
OAOAOA    const parentId = req.query.parentId || ROOT_FOLDER_ID.toString();
    const page = /\d+/.test((req.query.page || '').toString())
OAOAOAOAOAOAOAOAOAOAOAOA      ? Number.parseInt(req.query.page, 10)
      : 0;
OAOAOA    const filesFilter = {
      userId: user._id,
...
OAOAOA
OAOAOA    const files = await (await (await dbClient.filesCollection())
      .aggregate([
OAOAOAOAOAOA        { $match: filesFilter },
        { $sort: { _id: -1 } },
OAOAOA        { $skip: page * MAX_FILES_PER_PAGE },
...
OAOAOA        {
          $project: {
            ...
            isPublic: '$isPublic',
OAOAOA            parentId: {
OAOAOA              $cond: { if: { $eq: ['$parentId', '0'] }, then: 0, else: '$parentId' },
            },
OAOAOA          },
OAOAOAOAOAOA        },
      ])).toArray();
OAOAOA    res.status(200).json(files);
  }
